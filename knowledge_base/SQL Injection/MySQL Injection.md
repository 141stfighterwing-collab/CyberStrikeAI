...
In the GBK character set, the sequence `%df%5c` translates to the character `连`. So, the SQL query becomes: `SELECT * FROM users WHERE id='1连'' LIMIT 0,1`. Here, the wide byte character `连` effectively "eating" the added escape character, allowing for SQL injection.

Therefore, by using the payload `?id=1%df' and 1=1 --+`, after PHP adds the backslash, the SQL query transforms into: `SELECT * FROM users WHERE id='1连' and 1=1 --+' LIMIT 0,1`. This altered query can be successfully injected, bypassing the intended SQL logic.
