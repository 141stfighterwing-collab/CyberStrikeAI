    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                showError('Login is required to view API documentation. Please log in on the front-end page first, and then refresh this page.');
                return;
            }
    ...
    main.innerHTML = `
        <div class="empty-state">
            ...
            <div style="margin-top: 16px;">
                <a href="/" style="color: var(--accent-color); text-decoration: none;">Return to homepage login</a>
            </div>
        </div>
    `;
}
...
function renderAPIDocs() {
    if (!apiSpec || !apiSpec.paths) {
        showError('API specification format error');
        return;
    }
    
    // Show certification description
    renderAuthInfo();
    
    // Render sidebar grouping
    renderSidebar();
...
function renderAuthInfo() {
    ...
    if (currentToken && tokenStatus) {
        ...
        // If there is no token, display a prompt
        tokenStatus.style.display = 'block';
        tokenStatus.style.background = 'rgba(255, 152, 0, 0.1)';
        tokenStatus.style.borderLeftColor = '#ff9800';
        tokenStatus.innerHTML = '<p style="margin: 0; font-size: 0.8125rem; color: # Ff9800;"><strong>⚠ Token not detected</strong> - Please log in on the front-end page first, and then refresh this page. When testing the interface, you need to add Authorization: Bearer token</p>';
    }
}
...
function renderEndpoints(filterGroup = null) {
    ...
    if (endpoints.length === 0) {
        main.innerHTML = '<div class="empty-state"><h3>No API yet</h3><p>There is no API endpoint under this group</p></div>';
        return;
    }
    ...
    card.innerHTML = `
        ...
        <div class="api-endpoint-body">
            <div class="api-section">
                <div class="api-section-title">Describe</div>
...
function renderParameters(endpoint) {
    ...
    const rows = params.map(param => {
        ...
        // Process the description text, converting newlines to <br>
        let descriptionHtml = '-';
    ...
    return `
        <div class="api-section">
            <div class="api-section-title">Parameter</div>
...
function renderRequestBody(endpoint) {
    ...
    if (schema.properties) {
        ...
        const rows = Object.keys(schema.properties).map(key => {
            ...
            // Process description text, convert line breaks to <br> but keep other formatting
            let descriptionHtml = '-';
            if (prop.description) {
                // Escape HTML and then handle line breaks
                const escapedDesc = escapeHtml(prop.description);
                // Convert \n to <br>, but not escaped newlines
                descriptionHtml = escapedDesc.replace(/\n/g, '<br>');
            }
...
function renderTestSection(endpoint) {
    ...
    if (hasBody) {
        ...
        bodyInput = `
            <div class="api-test-input-group">
                ...
                <textarea id="${bodyInputId}" class="test-body-input" placeholder='Please enter the request body in JSON format'>${defaultBody}</textarea>
            </div>
        `;
    }
    ...
    return `
        <div class="api-test-section">
            <div class="api-section-title">Test interface</div>
...
Copy curl
                    ...
                    <button class="api-test-btn clear-result" onclick="clearTestResult('${escapeId(path)}-${method}')" title="Clear test results">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function testAPI(method, path, operationId) {
    ...
    try {
        ...
        // Make sure the path starts with /api (if the path in the OpenAPI specification does not contain /api)
        ...
        if (currentToken) {
            ...
            // If there is no token, prompt the user
            throw new Error('Token not detected. Please log in on the front-end page first, and then refresh this page. Or manually add Authorization: Bearer your_token in the request header');
        }
...
function copyCurlCommand(event, method, path) {
    try {
        ...
        if (['POST', 'PUT', 'PATCH'].includes(method.toUpperCase())) {
            ...
            if (bodyInput && bodyInput.value.trim()) {
                try {
                    // Validate and format JSON format
                    const jsonBody = JSON.parse(bodyInput.value.trim());
                    const jsonString = JSON.stringify(jsonBody);
                    // Within single quotes, only the single quote itself needs to be escaped
                    const escapedJson = jsonString.replace(/'/g, "'\\''");
                    curlCommand += ` \\\n  -d '${escapedJson}'`;
                } catch (e) {
                    // If it is not valid JSON, use the original value directly
        ...
        navigator.clipboard.writeText(curlCommand).then(() => {
            ...
            if (button) {
                ...
                alert('The curl command has been copied to the clipboard!');
            }
        }).catch(err => {
            console.error('Copy failed:', err);
            // If the clipboard API fails, use the fallback method
        ...
        console.error('Generating curl command failed:', error);
        alert('Generating curl command failed:' + error.message);
    }
}

// Format description text (processing markdown format)
function formatDescription(text) {
    if (!text) return '';
    
    // Extract the code block first (to prevent markdown within the code block from being processed)
    let formatted = text;
    const codeBlocks = [];
    let codeBlockIndex = 0;
    
    // Extract code chunks (supports language identifiers such as ```json or ```javascript)
    ...
    // Extract inline code (avoid markdown within inline code being processed)
    const inlineCodes = [];
    ...
    // Restore inline code (needs escaping because placeholders are already escaped)
    ...
    // Restore the code block (the content of the code block has been escaped and used directly)
    codeBlocks.forEach((block, index) => {
        const langLabel = block.lang ? `<span class="code-lang">${escapeHtml(block.lang)}</span>` : '';
        // The content of the code block has been saved during extraction and does not need to be escaped again
        formatted = formatted.replace(
    ...
    // Handle italics (*text* or _text_, but do not conflict with bold)
    formatted = formatted.replace(/(?<!\*)\*([^*\n]+?)\*(?!\*)/g, '<em>$1</em>');
    ...
    // Processing list items (ordered and unordered)
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (unorderedMatch) {
            ...
            if (line.trim()) {
                ...
                // Only add line breaks when not the last line
                result.push('<br>');
            }
        }
    }
...
function toggleDescription(button) {
    ...
    if (detail.style.display === 'none') {
        ...
        span.textContent = 'Hide detailed description';
    } else {
        detail.style.display = 'none';
        icon.style.transform = 'rotate(0deg)';
        span.textContent = 'View detailed instructions';
    }
}
