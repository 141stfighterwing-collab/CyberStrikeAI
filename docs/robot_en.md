...
| **List** or **Conversation List** (list) | List all conversation titles and IDs |
| **切换 \<conversationID\>** or **continue \<conversationID\>** | Continue in the given conversation |
| **New conversation** (new) | Start a new conversation |
| **Clear** (clear) | Clear current context (same effect as new conversation) |
| **current** (current) | Show current conversation ID and title |
| **Stop** (stop) | Abort the currently running task |
| **role** or **role list** (roles) | List all available roles (penetration testing, CTF, Web scan, etc.) |
| **角色 \<roleName\>** or **Switch role \<roleName\>** | Switch to the specified role |
| **删除 \<conversationID\>** | Delete the specified conversation |
| **version** (version) | Show current CyberStrikeAI version |
...
If the JSON response contains `"reply":"[CyberStrikeAI robot command]..."`, command handling works. You can also try `"text":"list"` or `"text":"current"`.
...
3. **Application logs**  
- On startup you should see: `DingTalk Stream is connecting...`, `DingTalk Stream has been started (no public network required), waiting to receive messages`.
- If you see `DingTalk Stream long connection exit` with an error, it’s usually wrong **Client ID / Client Secret** or **Stream not enabled** in the open platform.
- After sending a message in DingTalk, you should see `DingTalk received message` in the logs; if not, the platform is not pushing to this app (check that the bot is enabled and **Stream mode** is selected).
