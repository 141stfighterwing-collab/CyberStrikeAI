...
// Client uniformly encapsulates the HTTP client that interacts with OpenAI compatible models.
type Client struct {
...
// APIError represents a non-200 error returned by the OpenAI interface.
...
// UpdateConfig dynamically updates OpenAI configuration.
func (c *Client) UpdateConfig(cfg *config.OpenAIConfig) {
	c.config = cfg
}

// ChatCompletion calls the /chat/completions interface.
