...
func (h *AuthHandler) ChangePassword(c *gin.Context) {
	...
	if oldPassword == "" || newPassword == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Neither the current password nor the new password can be empty"})
		return
	}

	if len(newPassword) < 8 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "New password needs to be at least 8 characters long"})
		return
	}

	if oldPassword == newPassword {
		c.JSON(http.StatusBadRequest, gin.H{"error": "The new password cannot be the same as the old password"})
		return
	}

	if !h.manager.CheckPassword(oldPassword) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "The current password is incorrect"})
		return
	}
	...
	if err := h.manager.UpdateConfig(newPassword, h.config.Auth.SessionDurationHours); err != nil {
		if h.logger != nil {
			h.logger.Error("Failed to update authentication configuration", zap.Error(err))
		}
	...
	if h.logger != nil {
		h.logger.Info("The login password has been updated and all sessions have expired")
	}

	c.JSON(http.StatusOK, gin.H{"message": "The password has been updated, please use the new password to log in again"})
}
