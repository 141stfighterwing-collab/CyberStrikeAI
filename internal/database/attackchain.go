...
// AttackChainNode attack chain node
...
// AttackChainEdge attack chain edge
...
// SaveAttackChainNode saves the attack chain node
...
// SaveAttackChainEdge saves the attack chain edge
...
func (db *DB) LoadAttackChainNodes(conversationID string) ([]AttackChainNode, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			db.logger.Warn("Scanning attack chain nodes failed", zap.Error(err))
			continue
		}
...
func (db *DB) LoadAttackChainEdges(conversationID string) ([]AttackChainEdge, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			db.logger.Warn("Scanning attack chain edges failed", zap.Error(err))
			continue
		}
...
// DeleteAttackChain deletes the attack chain data of the conversation
func (db *DB) DeleteAttackChain(conversationID string) error {
	// Delete the edge first (because there is a foreign key constraint)
	_, err := db.Exec("DELETE FROM attack_chain_edges WHERE conversation_id = ?", conversationID)
	if err != nil {
		db.logger.Warn("Failed to delete attack chain edge", zap.Error(err))
	}

	// Delete the node again
	_, err = db.Exec("DELETE FROM attack_chain_nodes WHERE conversation_id = ?", conversationID)
	if err != nil {
		db.logger.Error("Failed to delete attack chain node", zap.Error(err), zap.String("conversationId", conversationID))
		return err
	}
