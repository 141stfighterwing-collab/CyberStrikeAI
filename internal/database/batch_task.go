...
// ListBatchQueues lists batch task queues (supports filtering and paging)
...
func (db *DB) CountBatchQueues(status, keyword string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("Failed to count the total number of batch task queues: %w", err)
	}

	return count, nil
}

// GetBatchTasks Gets all tasks in the batch task queue
func (db *DB) GetBatchTasks(queueID string) ([]*BatchTaskRow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Query batch task failed: %w", err)
	}
	defer rows.Close()
...
// UpdateBatchQueueCurrentIndex updates the current index of the batch task queue
func (db *DB) UpdateBatchQueueCurrentIndex(queueID string, currentIndex int) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to update current index of batch task queue: %w", err)
	}
	return nil
}

// UpdateBatchTaskMessage updates batch task message
...
func (db *DB) DeleteBatchQueue(queueID string) error {
	...
	// Delete tasks (foreign keys will be automatically deleted cascading)
